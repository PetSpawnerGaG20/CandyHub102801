...
local function buildGui()
	...
	local function buildInstaResetPanel()
		...
		UIS.InputChanged:Connect(function(input)
			if dragging and not uiLocked and dragStart and startPos and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then
	...
	local function buildAntiDesyncPanel()
		...
		local function toggleAntiDesyncFromPanel()
			...
			if tick()<panelJustOpenedUntil then return end
			if suppressNextClick then suppressNextClick=false;return end
			...
			if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
				...
				input.Changed:Connect(function()
					if input.UserInputState==Enum.UserInputState.End then
						...
						saveConfig()
						if moved then suppressNextClick=true;task.delay(0.2,function() suppressNextClick=false end) end
					end
				end)
			end
		end)
		...
		tapBtn.Activated:Connect(toggleAntiDesyncFromPanel)
		...
		setAntiDesyncPanelVisible=function(on,skipSave)
			...
			if antiDesyncPanelOpen then
				panelJustOpenedUntil=tick()+0.35
			end
			if not skipSave then saveConfig() end
		end
	end
